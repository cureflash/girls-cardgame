...
function playGame(a, b, { seed, aFirst, samples, maxActions, minGain }) {
  ...
  const kyokoTributes = engine.state.events.filter(event => event.type === 'opponentTribute');
  const kyokoSummons = engine.state.events.filter(event => event.type === 'summon' && event.special === 'kyoko');
  return {
    ...
    stats,
    kyoko: {
      tributeCount: kyokoTributes.length,
      tributePower: kyokoTributes.reduce((sum, event) => sum + (event.value ?? 0), 0),
      summonedPower: kyokoSummons.reduce((sum, event) => sum + (event.card?.attack ?? 0), 0),
    },
  };
}
...
function playPair(a, b, opts) {
  const result = {
    ...
    kyoko: { tributeCount: 0, tributePower: 0, summonedPower: 0 },
  };
  for (let game = 0; game < opts.games; game++) {
    ...
    for (const key of ['tributeCount', 'tributePower', 'summonedPower']) result.kyoko[key] += detail.kyoko[key];
  }
  result.winRates = { [a]: result.wins[a] / opts.games, [b]: result.wins[b] / opts.games };
  return result;
}

const output = arg('output', '/tmp/four-character-rollout.json');
const games = Number(arg('games', 100));
const samples = Number(arg('samples', 8));
const seed = Number(arg('seed', 2026091411));
const maxActions = Number(arg('max-actions', 300));
const minGain = Number(arg('min-gain', 0.05));
const characters = ['madoka', 'mami', 'sayaka', 'kyoko'];
const pairs = [
  ['madoka', 'mami'],
  ['madoka', 'sayaka'],
  ['mami', 'sayaka'],
...
const aggregate = Object.fromEntries(characters.map(id => [id, { games: 0, wins: 0, losses: 0, draws: 0 }]));
...
const report = {
  format: 'girls-cardgame-four-character-mutual-rollout-v1',
  ...
  deckCondition: 'Madoka, Mami, Sayaka, and Kyoko use mechanically identical 30-card decks; only names/art skins differ.',
...
console.log(JSON.stringify({
  pairs: pairResults.map(pair => ({ matchup: `${pair.a}-vs-${pair.b}`, wins: pair.wins, draws: pair.draws, truncated: pair.truncated, winRates: pair.winRates, stats: pair.stats, kyoko: pair.kyoko })),
  aggregate,
  output,
}, null, 2));
