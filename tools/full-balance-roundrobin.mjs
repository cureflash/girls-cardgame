...
for (let a = 0; a < roster.length; a++) {
  for (let b = a + 1; b < roster.length; b++) {
    ...
    for (let i = 0; i < half; i++) {
      const result = play([first, second], 2026091510 + pairIndex * 10000 + i);
    ...
    for (let i = 0; i < gamesPerPair - half; i++) {
      const result = play([second, first], 2026091510 + pairIndex * 10000 + 5000 + i);
      if (result.truncated) truncated++;
...
console.log('FULL_BALANCE_ROUNDROBIN=' + JSON.stringify({
  gamesPerPair,
