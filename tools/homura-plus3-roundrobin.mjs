...
function play(ids, seed) {
  ...
  return { winner: engine.state.phase === PHASES.GAME_OVER ? engine.state.winner : null, truncated: engine.state.phase !== PHASES.GAME_OVER };
}
...
for (let a = 0; a < roster.length; a++) {
  for (let b = a + 1; b < roster.length; b++) {
    ...
    for (let i = 0; i < half; i++) {
      const result = play([first, second], 2026091540 + pairIndex * 10000 + i);
    ...
    for (let i = 0; i < gamesPerPair - half; i++) {
      const result = play([second, first], 2026091540 + pairIndex * 10000 + 5000 + i);
      if (result.truncated) truncated++;
      if (result.winner === 1) firstWins++; else if (result.winner === 0) secondWins++; else draws++;
    }
    rows.push({ first, second, games: gamesPerPair, firstWins, secondWins, draws, truncated, firstWinRate: firstWins / gamesPerPair, secondWinRate: secondWins / gamesPerPair });
    standings[first].games += gamesPerPair; standings[first].wins += firstWins; standings[first].losses += secondWins; standings[first].draws += draws; standings[first].truncated += truncated;
    standings[second].games += gamesPerPair; standings[second].wins += secondWins; standings[second].losses += firstWins; standings[second].draws += draws; standings[second].truncated += truncated;
    pairIndex++;
  }
}
const table = Object.values(standings).map(item => ({ ...item, winRate: item.wins / item.games })).sort((a,b) => b.winRate - a.winRate || a.character.localeCompare(b.character));
console.log('HOMURA_PLUS3_ROUNDROBIN=' + JSON.stringify({ gamesPerPair, seatSplit:[Math.floor(gamesPerPair/2), gamesPerPair-Math.floor(gamesPerPair/2)], pairs:rows.length, totalGames:rows.reduce((s,r)=>s+r.games,0), rows, standings:table }));
