...
export const CHARACTERS = {
  ...
  sayaka: { id: 'sayaka', name: '美樹さやか', passive: '魔女召喚の生贄に必要な攻撃力合計を1減らす', special: '墓地の任意のカード3枚をデッキに戻す', image: null },
};
const SKINS = {
  madoka: {
  ...
  // Sayaka intentionally mirrors Madoka's numerical 30-card pool for the first balance study.
  // Only character abilities differ, so pairwise results isolate the new passive/special.
  sayaka: {
...
export function createDeck(characterId) {
  ...
  const tributeDiscount = characterId === 'sayaka' ? 1 : 0;
  for (const [code, name, attack, copies] of skin.witches) {
    add(`witch-${code}`, { name, type: CARD_TYPES.WITCH, attack, rank: attack, tributeThreshold: Math.max(0, attack - tributeDiscount), image: image(`witch_${code}`) }, copies);
  }
